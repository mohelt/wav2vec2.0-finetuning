...
class UrbanPredictor(nn.Module):
    def __init__(self, ssl_model, ssl_out_dim):
        super(UrbanPredictor, self).__init__()
    ...
    def forward(self, wav):
        ...
        x = torch.sigmoid(x)  ## add sigmoid activation function
        return x.squeeze(1)

    
class MyDataset(Dataset):
    def __init__(self, wavdir, urban_list):
        self.urban_lookup = { }
        f = open(urban_list, 'r')
        for line in f:
            ...
            wavname = parts[0]
            urban = float(parts[1])
            self.urban_lookup[wavname] = urban

        self.wavdir = wavdir
        self.wavnames = sorted(self.urban_lookup.keys())
    ...
    def __getitem__(self, idx):
        ...
        score = self.urban_lookup[wavname]
...
def main():
    ...
    trainlist = os.path.join(datadir, 'sets/URBAN/trainurban.txt')
    validlist = os.path.join(datadir, 'sets/URBAN/valurban.txt')
    ...
    net = UrbanPredictor(ssl_model, SSL_OUT_DIM)
    net = net.to(device)

    if my_checkpoint != None:  ## do (further) finetuning
        net.load_state_dict(torch.load(my_checkpoint,map_location="cuda" if torch.cuda.is_available() else "cpu"))
    
    criterion = nn.BCELoss()
    optimizer = optim.SGD(net.parameters(), lr=0.0001, momentum=0.9)

    PREV_VAL_LOSS=9999999999
    orig_patience=7 #change this value if you would like a higher patience values
    ...
    for epoch in range(1,1001):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            # Adjust sample of input sequences   
